...
def sharpe_bt(prices, rebalancing='Quarterly', lookback=90):
    """
    """
    df = prices.copy()
    ...
    for i in range(len(rebal)):
        ...
        weights = weights.join(pd.DataFrame(msr(df[start:end]), index=df.columns, columns=[rebal.index[i]]))
...
with st.expander('Sharpe Ratio Optimization Parameters'):
    ...
    lookback = d2.number_input('Lookback Period (in business days): ', min_value=90, max_value=252*3, value=90, step=1)

#if st.button("Run Maximum Sharpe Ratio Optimization"):
port = sharpe_bt(cust_df.dropna(), rebal_period, lookback)
